...
const ServiceCard = ({ title, description, imageUrl, button }) => {
  return (
    <div className=" relative flex flex-col items-start w-full max-w-[260px] sm:max-w-[280px]">
      ...
      {/* Card */}
      <div className="bg-[#7ef7dc] rounded-3xl px-5 pt-10 pb-10 text-left shadow-lg relative w-full min-h-[280px] sm:min-h-[240px]">
        <h3 className="text-[#1E0657] text-lg sm:text-xl font-bold mb-2">
          {title}
        </h3>
        <p className="text-[#0e0e0f] text-sm sm:text-base">{description}</p>
      </div>
